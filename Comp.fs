...
let rec cStmt stmt (varEnv : VarEnv) (funEnv : FunEnv) : instr list = 
    ...
    | Switch (e, l) ->
      let rec loop stmts =
            match stmts with
                | []     -> []
                | s1::sr -> let labelse = newLabel()
                            let labend  = newLabel()
                            let i=fst(s1)
                            cExpr e varEnv funEnv  @ [CSTI i] @ [EQ] @ [IFNZRO labelse]
                            @ loop sr @ [GOTO labend]
                            @ [Label labelse] @ cStmt snd(s1) varEnv funEnv 
                            @ [Label labend]
      loop l
    ...
    | For(x,estart,estop,stmt) ->
      let labbegin = newLabel()
      let labtest  = newLabel()
      cExpr x varEnv funEnv  @ [INCSP -1]
      @ [GOTO labtest] @ [Label labbegin] @ cStmt stmt varEnv funEnv @ cExpr estop varEnv funEnv @ [INCSP -1]
